...
func (r *quarktvBindingRepo) Get(ctx context.Context, accountID int64) (*domain.QuarkTVBinding, error) {
	...
	err := r.db.read.QueryRowContext(ctx,
		`SELECT account_id, device_id, refresh_token, access_token, token_expires_at, tv_uid, tv_nickname, bound_at
		 FROM quarktv_bindings WHERE account_id=?`, accountID).
		Scan(&b.AccountID, &b.DeviceID, &b.RefreshToken, &b.AccessToken, &expiresAt, &b.TVUID, &b.TVNickname, &boundAt)
...
func (r *quarktvBindingRepo) Upsert(ctx context.Context, b *domain.QuarkTVBinding) error {
	_, err := r.db.write.ExecContext(ctx,
		`INSERT INTO quarktv_bindings (account_id, device_id, refresh_token, access_token, token_expires_at, tv_uid, tv_nickname, bound_at)
		 VALUES (?,?,?,?,?,?,?,?)
		...
		b.AccountID, b.DeviceID, b.RefreshToken, b.AccessToken, b.TokenExpiresAt.Unix(), b.TVUID, b.TVNickname, b.BoundAt.Unix())
	return wrapDB(err)
}
